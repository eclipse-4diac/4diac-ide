...
public class ChangeOrganizationCommandTest extends CommandTestBase<ChangeOrganizationCommandTest.State> {
	...
	public static class State implements CommandTestBase.StateBase {
		...
		private ChangeOrganizationCommand cmd;

		public VersionInfo getVersionInfo() {
			return vInfo;
		}

		public ChangeOrganizationCommand getCommand() {
			return cmd;
		}

		public void setCommand(ChangeOrganizationCommand cmd) {
			this.cmd = cmd;
		}
	}
	...
	private static boolean verifyState(State state, State oldState) {
		return state.getVersionInfo().getOrganization().equals(NEW_ORGANIZATION)
	...
	private static boolean verifyStateNull(State state, State oldState) {
		return state.getVersionInfo().getOrganization().equals("") //$NON-NLS-1$
				&& state.getVersionInfo().getDate().equals(oldState.getVersionInfo().getDate())
				&& state.getVersionInfo().getRemarks().equals(oldState.getVersionInfo().getRemarks())
				&& state.getVersionInfo().getAuthor().equals(oldState.getVersionInfo().getAuthor())
				&& state.getVersionInfo().getVersion().equals(oldState.getVersionInfo().getVersion());
	...
	public static Collection<Object[]> data() {
		...
		commands.addAll(describeCommand("Start from default values", //
				...
				ChangeOrganizationCommandTest::createCommand, //
				executionDescriptions, //
				ChangeOrganizationCommandTest::undoCommand, //
				ChangeOrganizationCommandTest::redoCommand //
		));
