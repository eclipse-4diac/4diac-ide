...
public class ChangeVersionCommandTest extends CommandTestBase<ChangeVersionCommandTest.State> {
	...
	public static class State implements CommandTestBase.StateBase {
		...
		private ChangeVersionCommand cmd;

		public VersionInfo getVersionInfo() {
			return vInfo;
		}

		public ChangeVersionCommand getCommand() {
			return cmd;
		}

		public void setCommand(ChangeVersionCommand cmd) {
			this.cmd = cmd;
		}
	}
	...
	private static boolean verifyStateNull(State state, State oldState) {
		return state.getVersionInfo().getVersion().equals("") //$NON-NLS-1$
				&& state.getVersionInfo().getDate().equals(oldState.getVersionInfo().getDate())
				&& state.getVersionInfo().getOrganization().equals(oldState.getVersionInfo().getOrganization())
				&& state.getVersionInfo().getRemarks().equals(oldState.getVersionInfo().getRemarks())
				&& state.getVersionInfo().getAuthor().equals(oldState.getVersionInfo().getAuthor());
	...
	public static Collection<Object[]> data() {
		...
		commands.addAll(describeCommand("Start from default values", //
				...
				ChangeVersionCommandTest::createCommand, //
				executionDescriptions, //
				ChangeVersionCommandTest::undoCommand, //
				ChangeVersionCommandTest::redoCommand //
		));
