...
public class DeleteMemberVariableCommandTest extends DeleteMemberVariableCommandTestBase {
	...
	private static State executeDeletion(final State state, final VarDeclaration variable) {
		state.setCommand(new DeleteMemberVariableCommand(state.getStructuredType(), variable));

		return commandExecution(state);
	}

	private static void verifyDeletion(final State state, final State oldState, final TestFunction t, final VarDeclaration variable) {
	...
	public static Collection<Arguments> data() {
		final List<ExecutionDescription<?>> deletionExecutionDescriptions = List.of(
				new ExecutionDescription<>("Delete first member var", //$NON-NLS-1$
						(final State state) -> executeDeletion(state, varList.get(0)),
						(final State state, final State oldState, final TestFunction t) -> verifyDeletion(state, oldState, t,
								varList.get(0))),

				new ExecutionDescription<>("Delete middle member var", //$NON-NLS-1$
						(final State state) -> executeDeletion(state, varList.get(2)),
						(final State state, final State oldState, final TestFunction t) -> verifyDeletion(state, oldState, t,
								varList.get(2))),

				new ExecutionDescription<>("Delete last member var", //$NON-NLS-1$
						(final State state) -> executeDeletion(state, varList.get(varList.size() - 1)),
						(final State state, final State oldState, final TestFunction t) -> verifyDeletion(state, oldState, t,
								varList.get(varList.size() - 1))),

				new ExecutionDescription<>("Delete member struct", //$NON-NLS-1$
						(final State state) -> executeDeletion(state, varList.get(3)), (final State state, final State oldState,
								final TestFunction t) -> verifyDeletion(state, oldState, t, varList.get(3)))
