package org.eclipse.fordiac.ide.refactor.commands;
...
//	}
	...
	public ConnectionsToStructCommand(FBType sourceType, FBType destinationType, DataType structType, String sourceVarName, String destinationVarName, Map<String, String> conMap) {
		this(sourceType, destinationType, structType, sourceVarName, destinationVarName, conMap, true);
	}
	
	public ConnectionsToStructCommand(FBType sourceType, FBType destinationType, DataType structType, String sourceVarName, String destinationVarName, Map<String, String> conMap, boolean conflictResolution) {
...
//		}
	...
	private boolean editFBTypes() {
		...
		List<String> sourceWiths = sourceType.getInterfaceList().getOutputVars().stream()
				.filter(vardec -> replacableConMap.containsKey(vardec.getName()))
				.map(VarDeclaration::getWiths)
				.flatMap(List::stream)
				.map(width -> ((Event)width.eContainer()).getName())
				.distinct().toList();
		List<String> destinationWiths = destinationType.getInterfaceList().getInputVars().stream()
				.filter(vardec -> replacableConMap.containsValue(vardec.getName()))
				.map(VarDeclaration::getWiths)
				.flatMap(List::stream)
				.map(width -> ((Event)width.eContainer()).getName())
				.distinct().toList();
		...
		CreateInterfaceElementCommand sourceStruct = new CreateInterfaceElementCommand(structType, sourceVarName, sourceType.getInterfaceList(), false, (int)sourceType.getInterfaceList().getOutputs().count());
		CreateInterfaceElementCommand destinationStruct = new CreateInterfaceElementCommand(structType, destinationVarName, destinationType.getInterfaceList(), true, (int)destinationType.getInterfaceList().getInputs().count());
		editFBsCommand.add(sourceStruct);
		editFBsCommand.add(destinationStruct);
		
		editFBsCommand.execute();

		sourceWiths.forEach(width -> createWithsCommand.add(new WithCreateCommand(sourceType.getInterfaceList().getEvent(width),(VarDeclaration) sourceStruct.getCreatedElement())));
		destinationWiths.forEach(width -> createWithsCommand.add(new WithCreateCommand(destinationType.getInterfaceList().getEvent(width),(VarDeclaration) destinationStruct.getCreatedElement())));
...
//		destinationType = (FBType)destinationType.getTypeEntry().getType();
	...
	private boolean updateFBs() {
		...
		getElementsOfType(sourceType).stream()
	...
	private boolean createMUX() {
		...
		structConnectionMap = new HashMap<FBNetworkElement, FBNetworkElement>();
		demuxMap = new HashMap<FBNetworkElement, FBNetworkElement>();
		muxMap = new HashMap<FBNetworkElement, FBNetworkElement>();
		...
		getElementsOfType(destinationType).stream().forEach(instance -> {
			...
			if (cons.stream().map(con -> con.getSourceElement()).distinct().count() == 1 
				...
				muxcreate.execute();
				ChangeStructCommand changeStruct = new ChangeStructCommand((StructManipulator)muxcreate.getElement(), structType);
	...
	private boolean reconnectCon() {
		...
		getElementsOfType(destinationType).stream().forEach(instance -> {
			if (structConnectionMap.containsKey(instance)) {
				...
				//TODO: getInput if input is altered during creation (due to vars with same name)
	...
	private List<FBNetworkElement> getElementsOfType(FBType type) {
		return new BlockTypeInstanceSearch(type.getTypeEntry()).performSearch().stream().map(FBNetworkElement.class::cast).toList();
	}
}
