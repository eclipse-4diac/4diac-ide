...
public class ECCCreateStateDirectEditPolicy extends DirectEditPolicy {
	...
	public void performDirectEdit(final LocationRequest request) {
		creationLocation = request.getLocation().getCopy();
		new ECCStateCreationDirectEditManager(getHost(), creationLocation).show();
	}

	@Override
	protected Command getDirectEditCommand(final DirectEditRequest request) {
		if (creationLocation != null && request.getCellEditor() != null
				&& request.getCellEditor().getValue() instanceof final String name && !name.isBlank()) {
			final Point location = creationLocation.getCopy();
