...
public class STAlgorithmFilter {
	...
	private static final IResourceServiceProvider SERVICE_PRIVIDER = IResourceServiceProvider.Registry.INSTANCE
			.getResourceServiceProvider(STAlgorithmFilter.SYNTHETIC_ST_URI);
	...
	public String lua(final STAlgorithm alg) {
		try {
			...
			resource.load(_lazyStringInputStream, Collections
					.<String, Boolean>unmodifiableMap(CollectionLiterals.<String, Boolean>newHashMap(_mappedTo)));
			...
			if (_hasSyntaxErrors) {
				...
				Iterables.<String>addAll(this.errors,
						IterableExtensions.<INode, String>map(parseResult.getSyntaxErrors(), _function));
				return null;
			}
			...
			final Set<AdapterVariable> usedAdapterVariables = IteratorExtensions.<AdapterVariable>toSet(
					Iterators.<AdapterVariable>filter(EcoreUtil.<Object>getAllProperContents(stalg, true),
							AdapterVariable.class));
			...
			final Set<VarDeclaration> usedFBVariables = IteratorExtensions
					.<VarDeclaration>toSet(IteratorExtensions
							.<VarDeclaration>filter(IteratorExtensions.<PrimaryVariable, VarDeclaration>map(
									Iterators.<PrimaryVariable>filter(
											EcoreUtil.<Object>getAllProperContents(stalg, true), PrimaryVariable.class),
	...
	public CharSequence lua(final BasicFBType fb, final String expression) {
		try {
			...
			Pair<String, ParserRule> _mappedTo = Pair.<String, ParserRule>of(StructuredTextResource.OPTION_PARSER_RULE,
					_expressionRule);
			resource.load(_lazyStringInputStream, Collections
					.<String, ParserRule>unmodifiableMap(CollectionLiterals.<String, ParserRule>newHashMap(_mappedTo)));
			final IParseResult parseResult = resource.getParseResult();
			boolean _hasSyntaxErrors = parseResult.hasSyntaxErrors();
			if (_hasSyntaxErrors) {
				final Function1<INode, String> _function = (INode it) -> {
	...
	private CharSequence _luaExpression(final AdapterVariable expr) {
		StringConcatenation _builder = new StringConcatenation();
		CharSequence _luaAdapterVariable = LuaConstants.luaAdapterVariable(expr.getVar().getName(),
				expr.getAdapter().getName());
	...
	private CharSequence luaStatement(final Statement stmt) {
		if (stmt instanceof AssignmentStatement) {
			...
			throw new IllegalArgumentException("Unhandled parameter types: " + Arrays.<Object>asList(stmt).toString());
		}
	}
	...
	private CharSequence luaExpression(final Expression expr) {
		if (expr instanceof IntLiteral) {
			...
			throw new IllegalArgumentException("Unhandled parameter types: " + Arrays.<Object>asList(expr).toString());
		}
	}
