...
public class CompositeFBFilter {
	...
	public static CharSequence luaInternalFBs(final FBNetwork fbn) {
		...
		Iterable<FBNetworkElement> fbs = IterableExtensions.<FBNetworkElement>filter(fbn.getNetworkElements(),
				_function);
		_builder.newLineIfNotEmpty();
		{
	...
	public static CharSequence luaParameters(final FBNetwork fbn) {
		CharSequence _xblockexpression = null;
		{
			...
			Iterable<FBNetworkElement> fbs = IterableExtensions.<FBNetworkElement>filter(fbn.getNetworkElements(),
					_function);
			ArrayList<ArrayList<?>> parameters = CompositeFBFilter
					.getParameters(IterableExtensions.<FBNetworkElement>toList(fbs));
			StringConcatenation _builder = new StringConcatenation();
	...
	public static CharSequence luaEventConnections(final CompositeFBType type) {
		...
		final Function1<EventConnection, Boolean> _function = (EventConnection e) -> {
			return Boolean.valueOf(((e.getSource().getOutputConnections().size() == 1)
					|| ((e.getSource().getOutputConnections().size() > 1)
							&& e.getSource().getOutputConnections().get(0).equals(e))));
		};
			...
			for (final EventConnection con : connections) {
				...
				_builder.newLineIfNotEmpty();
				{
					if (((null != dne) && (null != sne))) {
						_builder.append("  ");
						_builder.append("{");
						String _luaConnectionString = CompositeFBFilter.luaConnectionString(sne, con.getSource(), type,
								"src");
						_builder.append(_luaConnectionString, "  ");
						_builder.append(", ");
						String _luaConnectionString_1 = CompositeFBFilter.luaConnectionString(dne, con.getDestination(),
								type, "dst");
						...
						if ((null == dne)) {
							_builder.append("  ");
							_builder.append("{");
							String _luaConnectionString_2 = CompositeFBFilter.luaConnectionString(sne, con.getSource(),
									type, "src");
							...
							if ((null == sne)) {
								...
								String _luaConnectionString_3 = CompositeFBFilter.luaConnectionString(dne,
										con.getDestination(), type, "dst");
	...
	public static String luaConnectionString(final FBNetworkElement e, final IInterfaceElement ev,
			final CompositeFBType type, final String stringID) {
		final Function1<AdapterDeclaration, AdapterFB> _function = (AdapterDeclaration it) -> {
			return it.getAdapterFB();
		};
		List<AdapterFB> plugs = IterableExtensions.<AdapterFB>toList(
				ListExtensions.<AdapterDeclaration, AdapterFB>map(type.getInterfaceList().getPlugs(), _function));
		final Function1<AdapterDeclaration, AdapterFB> _function_1 = (AdapterDeclaration it) -> {
			return it.getAdapterFB();
		};
		List<AdapterFB> sockets = IterableExtensions.<AdapterFB>toList(
				ListExtensions.<AdapterDeclaration, AdapterFB>map(type.getInterfaceList().getSockets(), _function_1));
		if ((e instanceof AdapterFB)) {
			...
			int _indexOf_1 = IterableExtensions
					.<FBNetworkElement>toList(IterableExtensions
							.<FBNetworkElement>filter(type.getFBNetwork().getNetworkElements(), _function_2))
					.indexOf(e);
	...
	public static CharSequence luaFannedOutEventConnections(final CompositeFBType type) {
		...
		List<EventConnection> conList = IterableExtensions
				.<EventConnection>toList(IterableExtensions.<EventConnection>filter(allCons, _function));
		_builder.newLineIfNotEmpty();
		_builder.append("  ");
			...
			for (final EventConnection con : connections) {
				...
				_builder.newLineIfNotEmpty();
				{
					if ((null != dne)) {
						...
						_builder.append(", ");
						String _luaConnectionString = CompositeFBFilter.luaConnectionString(dne, con.getDestination(),
								type, "dst");
						_builder.append(_luaConnectionString, "  ");
						_builder.append("}");
						_builder.newLineIfNotEmpty();
	...
	public static CharSequence luaFannedOutDataConnections(final CompositeFBType type) {
		...
		List<DataConnection> conList = IterableExtensions
				.<DataConnection>toList(IterableExtensions.<DataConnection>filter(allCons, _function));
		_builder.newLineIfNotEmpty();
		_builder.append("  ");
	...
	public static CharSequence luaFbnData(final FBNetwork fbn) {
		...
		int _size = IterableExtensions
				.size(IterableExtensions.<FBNetworkElement>filter(fbn.getNetworkElements(), _function));
		...
		int _size_1 = IterableExtensions
				.size(IterableExtensions.<EventConnection>filter(fbn.getEventConnections(), _function_1));
		...
		int _size_2 = IterableExtensions
				.size(IterableExtensions.<EventConnection>filter(fbn.getEventConnections(), _function_2));
		...
		int _size_3 = IterableExtensions
				.size(IterableExtensions.<DataConnection>filter(fbn.getDataConnections(), _function_3));
		...
		int _size_4 = IterableExtensions
				.size(IterableExtensions.<DataConnection>filter(fbn.getDataConnections(), _function_4));
	...
	private static int getNumParameter(final FBNetwork fbn) {
		...
		return CompositeFBFilter.getParameters(IterableExtensions.<FBNetworkElement>toList(
				IterableExtensions.<FBNetworkElement>filter(fbn.getNetworkElements(), _function))).size();
	}
	...
	private static ArrayList<ArrayList<?>> getParameters(final List<FBNetworkElement> fbs) {
		...
		for (final FBNetworkElement ne : fbs) {
			...
			for (final VarDeclaration iv : _inputVars) {
				if ((((null != iv.getValue()) && (null != iv.getValue().getValue()))
						&& (!iv.getValue().getValue().isEmpty()))) {
