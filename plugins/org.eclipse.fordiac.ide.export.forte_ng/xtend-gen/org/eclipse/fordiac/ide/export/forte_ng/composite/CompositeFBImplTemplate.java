...
public class CompositeFBImplTemplate extends ForteFBTemplate {
	...
	protected CharSequence generateFBNetwork() {
		StringConcatenation _builder = new StringConcatenation();
		{
			...
			boolean _exists = IterableExtensions.<FBNetworkElement>exists(this.type.getFBNetwork().getNetworkElements(),
					_function);
			if (_exists) {
				...
				_builder.append("  ");
				{
					...
					Iterable<FBNetworkElement> _filter = IterableExtensions
							.<FBNetworkElement>filter(this.type.getFBNetwork().getNetworkElements(), _function_1);
			...
			if (_not) {
				...
				_builder.append("  ");
				{
					...
					for (final EventConnection conn : _eventConnections) {
						...
						_builder.append("{");
						String _generateConnectionPortID = this.generateConnectionPortID(conn.getSource(),
								conn.getSourceElement());
						_builder.append(_generateConnectionPortID, "  ");
						_builder.append(", ");
						String _generateConnectionPortID_1 = this.generateConnectionPortID(conn.getDestination(),
								conn.getDestinationElement());
			...
			if (_not_1) {
				...
				_builder.append("  ");
				{
					...
					for (final DataConnection conn_1 : _dataConnections) {
						...
						_builder.append("{");
						String _generateConnectionPortID_2 = this.generateConnectionPortID(conn_1.getSource(),
								conn_1.getSourceElement());
						_builder.append(_generateConnectionPortID_2, "  ");
						_builder.append(", ");
						String _generateConnectionPortID_3 = this.generateConnectionPortID(conn_1.getDestination(),
								conn_1.getDestinationElement());
		...
		int _size = IterableExtensions.size(IterableExtensions
				.<FBNetworkElement>filter(this.type.getFBNetwork().getNetworkElements(), _function_2));
		...
		_builder.append(", ");
		{
			...
			boolean _exists_1 = IterableExtensions
					.<FBNetworkElement>exists(this.type.getFBNetwork().getNetworkElements(), _function_3);
			if (_exists_1) {
				_builder.append("scm_astInternalFBs");
			} else {
