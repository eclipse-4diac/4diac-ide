...
public class ForteNgExportFilter extends TemplateExportFilter {
	...
	protected Set<? extends IExportTemplate> getTemplates(final LibraryElement type) {
		...
		if (type instanceof BasicFBType) {
			...
			_builder.append(".h");
			Path _get = Paths.get("");
			BasicFBHeaderTemplate _basicFBHeaderTemplate = new BasicFBHeaderTemplate(((BasicFBType) type),
					_builder.toString(), _get);
			StringConcatenation _builder_1 = new StringConcatenation();
			String _name_1 = ((BasicFBType) type).getName();
			_builder_1.append(_name_1);
			_builder_1.append(".cpp");
			Path _get_1 = Paths.get("");
			BasicFBImplTemplate _basicFBImplTemplate = new BasicFBImplTemplate(((BasicFBType) type),
					_builder_1.toString(), _get_1);
			_switchResult = Collections.<IExportTemplate>unmodifiableSet(
					CollectionLiterals.<IExportTemplate>newHashSet(_basicFBHeaderTemplate, _basicFBImplTemplate));
		}
		if (!_matched) {
			if (type instanceof SimpleFBType) {
				...
				_builder.append(".h");
				Path _get = Paths.get("");
				SimpleFBHeaderTemplate _simpleFBHeaderTemplate = new SimpleFBHeaderTemplate(((SimpleFBType) type),
						_builder.toString(), _get);
				StringConcatenation _builder_1 = new StringConcatenation();
				...
				_builder_1.append(".cpp");
				Path _get_1 = Paths.get("");
				SimpleFBImplTemplate _simpleFBImplTemplate = new SimpleFBImplTemplate(((SimpleFBType) type),
						_builder_1.toString(), _get_1);
				_switchResult = Collections.<IExportTemplate>unmodifiableSet(
						CollectionLiterals.<IExportTemplate>newHashSet(_simpleFBHeaderTemplate, _simpleFBImplTemplate));
			}
		}
		if (!_matched) {
			if (type instanceof CompositeFBType) {
				...
				_builder.append(".h");
				Path _get = Paths.get("");
				CompositeFBHeaderTemplate _compositeFBHeaderTemplate = new CompositeFBHeaderTemplate(
						((CompositeFBType) type), _builder.toString(), _get);
				StringConcatenation _builder_1 = new StringConcatenation();
				String _name_1 = ((CompositeFBType) type).getName();
				_builder_1.append(_name_1);
				_builder_1.append(".cpp");
				Path _get_1 = Paths.get("");
				CompositeFBImplTemplate _compositeFBImplTemplate = new CompositeFBImplTemplate(((CompositeFBType) type),
						_builder_1.toString(), _get_1);
				_switchResult = Collections.<IExportTemplate>unmodifiableSet(CollectionLiterals
						.<IExportTemplate>newHashSet(_compositeFBHeaderTemplate, _compositeFBImplTemplate));
			}
		}
		if (!_matched) {
			if (type instanceof AdapterType) {
				...
				_builder.append(".h");
				Path _get = Paths.get("");
				AdapterFBHeaderTemplate _adapterFBHeaderTemplate = new AdapterFBHeaderTemplate(_adapterFBType,
						_builder.toString(), _get);
				...
				_builder_1.append(".cpp");
				Path _get_1 = Paths.get("");
				AdapterFBImplTemplate _adapterFBImplTemplate = new AdapterFBImplTemplate(_adapterFBType_1,
						_builder_1.toString(), _get_1);
				_switchResult = Collections.<IExportTemplate>unmodifiableSet(CollectionLiterals
						.<IExportTemplate>newHashSet(_adapterFBHeaderTemplate, _adapterFBImplTemplate));
			}
		}
		if (!_matched) {
			if (type instanceof ServiceInterfaceFBType) {
				...
				_builder.append(".h");
				Path _get = Paths.get("");
				ServiceInterfaceFBHeaderTemplate _serviceInterfaceFBHeaderTemplate = new ServiceInterfaceFBHeaderTemplate(
						((ServiceInterfaceFBType) type), _builder.toString(), _get);
				StringConcatenation _builder_1 = new StringConcatenation();
				String _name_1 = ((ServiceInterfaceFBType) type).getName();
				_builder_1.append(_name_1);
				_builder_1.append(".cpp");
				Path _get_1 = Paths.get("");
				ServiceInterfaceFBImplTemplate _serviceInterfaceFBImplTemplate = new ServiceInterfaceFBImplTemplate(
						((ServiceInterfaceFBType) type), _builder_1.toString(), _get_1);
				_switchResult = Collections.<IExportTemplate>unmodifiableSet(
						CollectionLiterals.<IExportTemplate>newHashSet(_serviceInterfaceFBHeaderTemplate,
								_serviceInterfaceFBImplTemplate));
			}
		}
		if (!_matched) {
