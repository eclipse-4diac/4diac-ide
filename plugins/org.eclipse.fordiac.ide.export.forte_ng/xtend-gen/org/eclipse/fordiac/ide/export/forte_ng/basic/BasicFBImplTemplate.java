...
public class BasicFBImplTemplate extends ForteFBTemplate {
	...
	protected CharSequence generateECC() {
					...
					for (final ECTransition transition : _outTransitions) {
						...
						}
						{
							if (((transition.getConditionEvent() != null)
									&& (!StringExtensions.isNullOrEmpty(transition.getConditionExpression())))) {
								_builder.append("      ");
								_builder.append("  ");
								...
								CharSequence _generate = this.stAlgorithmFilter
										.generate(transition.getConditionExpression(), this.type, this.getErrors());
								_builder.append(_generate, "        ");
								_builder.append("))");
								_builder.newLineIfNotEmpty();
							} else {
								...
								if (_tripleNotEquals) {
									...
									boolean _isNullOrEmpty = StringExtensions
											.isNullOrEmpty(transition.getConditionExpression());
									boolean _not = (!_isNullOrEmpty);
									if (_not) {
										_builder.append("      ");
										_builder.append("  ");
										_builder.append("if(");
										CharSequence _generate_1 = this.stAlgorithmFilter.generate(
												transition.getConditionExpression(), this.type, this.getErrors());
		...
		_builder.append("        ");
		_builder.append(
				"DEVLOG_ERROR(\"The state is not in the valid range! The state value is: %d. The max value can be: ");
	...
	protected CharSequence generateAlgorithm(final Algorithm alg) {
		if (alg instanceof STAlgorithm) {
			...
			throw new IllegalArgumentException("Unhandled parameter types: " + Arrays.<Object>asList(alg).toString());
		}
	}
	...
	protected CharSequence generateSendEvent(final Event event) {
		if (event instanceof AdapterEvent) {
			...
			throw new IllegalArgumentException("Unhandled parameter types: " + Arrays.<Object>asList(event).toString());
		}
	}
