...
public class TestEccGenerator {
	...
	public void createTransitionFromTo(ECState from, ECState to, Event input) {
	...
	public void createState(String name, int stateCount) {
	...
	public Algorithm createMatchAlgorithm(BasicFBType fb, List<VarDeclaration> outputData, String name) { //String expected, String received
		...
		for (VarDeclaration varDecl : outputData) {			
			text.append("if "+ varDecl.getName() +"_expected = "+ varDecl.getName() +"_received then\n");
			text.append(varDecl.getName() +"_matchData := true;\n");
			text.append("else\n");
			text.append(varDecl.getName() +"_matchData := false;\n");
			text.append("end_if;\n");
		}
	...
	public Algorithm createTestFbAlgortihm(BasicFBType fb, TestSuite testSuite, String name) {
		final TextAlgorithm alg = LibraryElementFactory.eINSTANCE.createSTAlgorithm();
		alg.setName(name);
		fb.getCallables().add(alg);
		return alg;
	...
	public void createState(TestCase testCase, int stateCount) {
		final ECState state = LibraryElementFactory.eINSTANCE.createECState();
		ecc.getECState().add(state);
