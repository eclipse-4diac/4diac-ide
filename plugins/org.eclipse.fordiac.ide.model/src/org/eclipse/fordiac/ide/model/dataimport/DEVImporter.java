...
public final class DEVImporter {
	 ...
	 */
	private static DeviceType parseDEVType(final DeviceType type, final Node rootNode, final Palette palette)
	...
	private static Resource parseResource(final Node node, final Palette palette) throws TypeImportException {
		...
		if (type != null) {
			...
			if (type.getNodeValue().contains("/")) { //$NON-NLS-1$
				entry = palette.getTypeEntryForPath(type.getNodeValue(),
						TypeLibraryTags.RESOURCE_TYPE_FILE_ENDING_WITH_DOT);
			} else {
	...
	private static void parseDeviceTypeAttributeDeclaration(DeviceType device, Node node) {
		...
		attributeDeclaration
				.setInitialValue(attributeMap.getNamedItem(LibraryElementTags.INITIALVALUE_ATTRIBUTE).getNodeValue());
		attributeDeclaration.setType(
				BaseType1.getByName(attributeMap.getNamedItem(LibraryElementTags.TYPE_ATTRIBUTE).getNodeValue()));
