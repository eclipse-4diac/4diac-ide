...
public class GlobalConstantsResourceSetInitializer implements IResourceSetInitializer {
	...
	public static Injector getInjector() {
		final GlobalconstantseditorActivator activator = GlobalconstantseditorActivator.getInstance();
		if (activator == null) {
			throw new IllegalStateException("The bundle has not been started!"); //$NON-NLS-1$
		}
		final Injector injector = activator.getInjector(
				GlobalconstantseditorActivator.ORG_ECLIPSE_FORDIAC_IDE_GLOBALCONSTANTSEDITOR_GLOBALCONSTANTS);
		if (injector == null) {
