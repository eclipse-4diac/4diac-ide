...
public class ToggleWatchSubElementsHandler extends AbstractHandler implements IElementUpdater {

	@Override
	public Object execute(final ExecutionEvent event) throws ExecutionException {
		getWatches(HandlerUtil.getCurrentStructuredSelection(event))
				.forEachOrdered(watch -> toggleWatchSubElements(watch, event));
		return null;
	}

	private static void toggleWatchSubElements(final IWatch watch, final ExecutionEvent event) {
	...
	private static void createWatchpoint(final IResource resource, final ITypedElement element,
		...
		try {
			...
			watchpoint.setWatchSubElements(true);
	...
	private static void updateWatchpoint(final DeploymentWatchpoint watchpoint, final ExecutionEvent event) {
		try {
			watchpoint.setWatchSubElements(!watchpoint.isWatchSubElements());
		} catch (final CoreException e) {
			ErrorDialog.openError(HandlerUtil.getActiveShell(event), null, null, e.getStatus());
		}
	}
	...
	public void setEnabled(final Object evaluationContext) {
		setBaseEnabled(HandlerUtil.getVariable(evaluationContext,
				...
				&& getWatches(selection).allMatch(ToggleWatchSubElementsHandler::isValidWatch));
	}
	...
	public void updateElement(final UIElement element, final Map parameters) {
		...
		if (HandlerUtil.getVariable(evaluationContext,
				ISources.ACTIVE_CURRENT_SELECTION_NAME) instanceof final IStructuredSelection selection) {
			if (getWatches(selection).noneMatch(ISubContainerWatch::isWatchSubElements)) {
				element.setText(Messages.ToggleWatchSubElementsHandler_WatchSubElements);
			} else if (getWatches(selection).allMatch(ISubContainerWatch::isWatchSubElements)) {
				element.setText(Messages.ToggleWatchSubElementsHandler_RemoveSubElementWatches);
			}
		}
	}

	private static Stream<ISubContainerWatch> getWatches(final IStructuredSelection selection) {
		return selection.stream().filter(ISubContainerWatch.class::isInstance).map(ISubContainerWatch.class::cast);
	}
}
