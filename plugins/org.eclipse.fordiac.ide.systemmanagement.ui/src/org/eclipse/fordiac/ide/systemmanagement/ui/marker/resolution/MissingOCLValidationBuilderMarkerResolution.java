...
public class MissingOCLValidationBuilderMarkerResolution extends WorkspaceMarkerResolution {
	...
	protected void runInWorkspace(final IMarker[] markers, final IProgressMonitor monitor) throws CoreException {
		...
		for (final IMarker marker : markers) {
			...
			if (FordiacNature.configureOCLValidationBuilder(description)) {
	...
	public String getLabel() {
		return Messages.MissingOCLValidationBuilderMarkerResolution_Label;
	}

	@Override
	public String getDescription() {
		return Messages.MissingOCLValidationBuilderMarkerResolution_Description;
	}
	...
	public IMarker[] findOtherMarkers(final IMarker[] markers) {
		return Stream.of(markers)
				.filter(other -> FordiacNature.class.getName().equals(FordiacErrorMarker.getSource(other))
						&& FordiacNature.MISSING_OCL_VALIDATION_BUILDER == FordiacErrorMarker.getCode(other))
				.toArray(IMarker[]::new);
	}
}
