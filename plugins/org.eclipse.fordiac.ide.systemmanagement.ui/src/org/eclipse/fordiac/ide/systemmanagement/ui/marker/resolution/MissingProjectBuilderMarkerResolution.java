...
public class MissingProjectBuilderMarkerResolution extends WorkspaceMarkerResolution {
	@Override
	protected void runInWorkspace(final IMarker[] markers, final IProgressMonitor monitor) throws CoreException {
		...
		for (final IMarker marker : markers) {
			...
			if (FordiacNature.configureProjectBuilders(description)) {
	...
	public String getLabel() {
		return Messages.MissingProjectBuilderMarkerResolution_Label;
	}

	@Override
	public String getDescription() {
		return Messages.MissingProjectBuilderMarkerResolution_Description;
	}
	...
	public IMarker[] findOtherMarkers(final IMarker[] markers) {
		return Stream.of(markers)
				.filter(other -> FordiacNature.class.getName().equals(FordiacErrorMarker.getSource(other))
						&& FordiacNature.MISSING_PROJECT_BUILDER == FordiacErrorMarker.getCode(other))
				.toArray(IMarker[]::new);
	}
}
