...
public final class AddStructMemberRefactoring extends Refactoring {
	...
	public String getName() {
		return MessageFormat.format(Messages.AddStructMemberRefactoring_Name, configuration.memberName(),
				context.getStructTypeFileName());
	}

	@Override
	public RefactoringStatus checkInitialConditions(final IProgressMonitor pm)
			throws CoreException, OperationCanceledException {
		final RefactoringStatus status = checkContextConditions();
		if (!status.hasFatalError()) {
			addMultipleUsesWarning(status);
		}
		pm.done();
		return status;
	}

	private RefactoringStatus checkContextConditions() {
		final RefactoringStatus status = new RefactoringStatus();
		final TypeEntry structTypeEntry = StructMemberRefactoringSupport.getTypeEntry(context.getStructTypeURI());

		if (!(structTypeEntry instanceof DataTypeEntry)
		...
		if (context.hasConnection()) {
			...
			if (targetTypeEntry == null) {
				status.addFatalError(Messages.AddStructMemberRefactoring_InvalidContext);
			} else if (!StructMemberRefactoringSupport.isWritable(targetTypeEntry)) {
	...
	private void addMultipleUsesWarning(final RefactoringStatus status) {
		final TypeEntry typeEntry = StructMemberRefactoringSupport.getTypeEntry(context.getStructTypeURI());
		if (typeEntry instanceof final DataTypeEntry dataTypeEntry) {
			final int useCount = DataTypeInstanceSearch.createSearchIncludingDerivedDataTypes(dataTypeEntry).performSearch()
					.size();
			if (useCount > 1) {
				status.addWarning(MessageFormat.format(Messages.AddStructMemberRefactoring_MultipleUsesWarning,
						Integer.valueOf(useCount)));
			}
		}
	}
	...
	private RefactoringStatus validateConfiguration(final AddStructMemberConfiguration candidate) {
		...
		if (context.hasConnection() && !context.getConnectionArraySize().equals(candidate.arraySize())) {
			status.addFatalError(Messages.AddStructMemberRefactoring_InvalidContext);
		}
