...
public class CommandRedoChange<T extends EObject> extends AbstractCommandChange<T> {

	private final Command command;
	private final LibraryElement libraryElement;

	public CommandRedoChange(final String name, final URI elementURI, final Class<T> elementClass,
	...
	public RefactoringStatus isValid(final T element, final IProgressMonitor pm)
		...
		if ((commandStack != null && commandStack.getRedoCommand() != command) || !command.canRedo()) {
			status.addFatalError(Messages.CommandRedoChange_CannotRedoCommand);
		}
		return status;
	}
	...
	protected Command createCommand(final T element) {
		// we need an existing command to redo
	...
	protected Change createUndoChange(final Command command, final LibraryElement libraryElement) {
		return new CommandUndoChange<>(getName(), getElementURI(), getElementClass(), command, libraryElement);
	}
}
