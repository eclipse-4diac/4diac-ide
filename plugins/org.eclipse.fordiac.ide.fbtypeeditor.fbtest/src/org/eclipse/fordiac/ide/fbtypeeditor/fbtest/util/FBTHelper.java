			...
			if (null != st.getInputPrimitive()) {
				...
				if (0 <= eventID) {
					TestPrimitive tp = new TestPrimitive(
							new org.eclipse.fordiac.ide.fbtypeeditor.fbtest.util.Event(event, eventID, true));
					...
					if (useInputQualifier) {
						...
						if (0 <= qiID) {
							...
							DataVariable qi = new DataVariable("QI", qiID, qiDT, true, //$NON-NLS-1$
									Boolean.toString(inputQualifier));
							tp.addData(qi);
						}
					}
			...
			if (null != st.getOutputPrimitive()) {
				if (!st.getOutputPrimitive().isEmpty()) {
					for (Iterator<OutputPrimitive> iterator = st.getOutputPrimitive().iterator(); iterator.hasNext();) {
						...
						if (0 <= eventID) {
							tp.setEvent(
									new org.eclipse.fordiac.ide.fbtypeeditor.fbtest.util.Event(event, eventID, false));
							...
							if (useOutputQualifier) {
								...
								if (0 <= qoID) {
									...
									DataVariable qo = new DataVariable("QO", qoID, qoDT, false, //$NON-NLS-1$
											Boolean.toString(outputQualifier));
									tp.addData(qo);
								}
							}
						}
	...
	public static int getMaxOutputPrimitives(List<TestSequence> paTests) {
		...
		for (TestSequence ts : paTests) {
			if ((ts.getTestTransactions() != null) && (!ts.getTestTransactions().isEmpty())) {
				for (Iterator<TestTransaction> TTiterator = ts.getTestTransactions().iterator(); TTiterator
						.hasNext();) {
